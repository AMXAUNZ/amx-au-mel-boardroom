...
define_function resetAllDraggablePopups (dev dragAndDropVirtual)
{
    select
    {
		...
		{
			hideDraggablePopup (vdvDragAndDropTpTable, dvDvxVidInTv.port)
			...
			showDraggablePopup (vdvDragAndDropTpTable, dvDvxVidInTv.port)
...
define_function addDragItemsAll (dev dragAndDropVirtual)
{
	...
	active (dragAndDropVirtual == vdvDragAndDropTpTable):
	{
	    addDragItem (vdvDragAndDropTpTable, dvDvxVidInTv.port)
...
define_function enableDragItemsAll (dev dragAndDropVirtual)
{
	select
	{
		active (dragAndDropVirtual == vdvDragAndDropTpTable):
		{
			enableDragItem (vdvDragAndDropTpTable, dvDvxVidInTv.port)
...
define_function disableDragItemsAll (dev dragAndDropVirtual)
{
	select
	{
		active (dragAndDropVirtual == vdvDragAndDropTpTable):
		{
			disableDragItem (vdvDragAndDropTpTable, dvDvxVidInTv.port)
...
define_function tableInputDetected (dev dvTxVidIn)
{
	...
	if (!isSystemAvInUse)
	{
		...
		moderoEnablePopup (dvTpTableMain, POPUP_NAME_DRAGGABLE_SOURCES[dvDvxVidInTv.port])
		...
		moderoEnablePopup (dvTpTableMain, POPUP_NAME_DRAGGABLE_SOURCES[dvDvxVidInSignage.port])
